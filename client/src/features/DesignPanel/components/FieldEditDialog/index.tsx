...
import { selectedSeedBedSelector } from "../../selectors";
...
const FieldEditDialog: React.FC<IFieldEditDialogProps> = (props) => {
    ...
    const actualSeedBed = useAppSelector(selectedSeedBedSelector);
    ...
    const cropChanged = (e: React.MouseEvent) => {
        ...
        if (newPlant) {
            dispatch(appActions.changePlantAction(newPlant));
        }
    }
    ...
    const varietyChanged = (e: React.MouseEvent) => {
        ...
        if (newVariety) {
            dispatch(appActions.changeVarietyAction(newVariety));
        }
    }

    const plantSpacingChanged = (e: React.ChangeEvent<HTMLInputElement>) => {
        dispatch(appActions.updateSpacingAction({ id: actualSeedBed.id, plantSpacing: parseInt(e.currentTarget.value), rowSpacing }))
    }

    const rowSpacingChanged = (e: React.ChangeEvent<HTMLInputElement>) => {
        dispatch(appActions.updateSpacingAction({ id: actualSeedBed.id, plantSpacing, rowSpacing: parseInt(e.currentTarget.value) }))
    }
    ...
    const spacingTypeChangedEventHandler = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        if (option === 0) {
            dispatch(appActions.updateSpacingAction({ id: actualSeedBed.id, plantSpacing: actualSeedBed.plant.PlantSpacingMin, rowSpacing: actualSeedBed.plant.RowSpacingMin }))
        } else if (option === 1) {
            ...
            dispatch(appActions.updateSpacingAction({ id: actualSeedBed.id, plantSpacing, rowSpacing }))
        } else {
            dispatch(appActions.updateSpacingAction({ id: actualSeedBed.id, plantSpacing, rowSpacing }))
        }
    }
        ...
        `}>
            <ModalWindow position={{ left: "50%", top: "50%" }} dimension={{
                width: (modalWidth + 30) + "px", height
                    : "initial"
            }} closeModalHandler={() => { dispatch(appActions.updateSelectedSeedBedAction(-1)) }}>
                <Label text={"Plant"}>
