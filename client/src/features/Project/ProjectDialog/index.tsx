...
import { IAppSlice, appActions } from '../../../store/reducers/AppSlice';
...
import { lastModifiedSelector, projectIDSelector, projectNameSelector } from '../selectors';

export interface IProjectInfos extends IAppSlice{
    onClick?: any
}
...
const ProjectDialog: React.FC<IProjectDialogProps> = (props) => {
    ...
    const projectClickedHandler = useCallback((clickedItem: IProjectInfos) => {
        if (clickedItem && clickedItem?.project.projectName?.length) {
            ...
            dispatch(appActions.setProjectAction(projectData));
        }
    ...
    const saveProject = () => {
        dispatch(appActions.setLMTAction(new Date().getTime()))
        dispatch(hideProjectDialogAction());
    }
    ...
    useEffect(() => {
        if(projectID === -1){
            ...
            dispatch(appActions.setProjectIDAction(projID))
        }else{
        ...
        : "500px"}} closeModalHandler={() => dispatch(hideProjectDialogAction())}>
            ...
            {props.state === ProjectDialogStates.SAVE_PROJECT && (
                <div css={css`
                    display: flex;
                `}>
                    <InputField value={projectName} placeholder='Název projektu' onChangeHandler={(e)=>{dispatch(appActions.setProjectNameAction(e.target.value))}} />
